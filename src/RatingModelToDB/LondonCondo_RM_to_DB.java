...
public class LondonCondo_RM_to_DB {
	...
	public static void main(String[] args) throws Exception {
		...
		FileInputStream fis = new FileInputStream("D:\\Rate Matchers\\Condo Rating Model - version TBD 11.3.21.xlsx");
		XSSFWorkbook workbook = new XSSFWorkbook(fis);
		List<String> sheetNames = new ArrayList<String>();
		List<String> rateFactors = new ArrayList<String>();
		logger.info(Thread.currentThread().getStackTrace()[1].getLineNumber() + ": STARTED FOR One London Condo RATER ");
		...
		/*
		 * Removing the elements in the array which do not have the sheet name LondonCondo_Rater
		 */
		for (int i = 0; i < sheetNames.size(); i++) {
			sheetNames.removeIf(n -> (!n.contains("LondonCondo_Rater ")));
		}
		Sheet rowsSheet = workbook.getSheet("Output_rater");
		for (int s_no = 0; s_no < sheetNames.size(); s_no++) {
			logger.info(Thread.currentThread().getStackTrace()[1].getLineNumber() + ": Going to sheet "
					+ sheetNames.get(s_no));
			Sheet ResultSheet = workbook.getSheet(sheetNames.get(s_no));
			...
			String query = getQuery.LondonCondo_RM_to_DB(s_no, rateFactors);
			logger.info(
