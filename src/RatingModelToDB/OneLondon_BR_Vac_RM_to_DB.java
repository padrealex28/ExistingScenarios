...
public class OneLondon_BR_Vac_RM_to_DB {
	...
	public static void main(String[] args) throws Exception {
		...
		FileInputStream fis = new FileInputStream("D:\\Rate Matchers\\Alex One London - BR and VAC - V2.xlsx");
		XSSFWorkbook workbook = new XSSFWorkbook(fis);
		List<String> sheetNames = new ArrayList<String>();
		List<String> rateFactors = new ArrayList<String>();
		logger.info(Thread.currentThread().getStackTrace()[1].getLineNumber() + ": STARTED FOR One London BR/VAC RATER ");
		...
		for (int i = 0; i < workbook.getNumberOfSheets(); i++) 
		{
		 ...
		 /* Removing the elements in the array which do not have the sheet name OneLondon_Rater */
		 
		for (int i = 0; i < sheetNames.size(); i++) 
		{
			sheetNames.removeIf(n -> (!n.contains("OneLondon_Rater ")));
		}
		...
		for (int i = 0; i < sheetNames.size(); i++) 
		{
			...
			String query = getQuery.OneLondon_BR_Vac_RM_to_DB(i+11, rateFactors);
		...
		logger.info("\n");
		logger.info("COPIED TO DB,COMPARISON IN PROGRESS..!!!");
		Comparison_Queries cq = new Comparison_Queries();
		cq.ComparePremium_OneLondon_BR_Vac();	
		logger.info("\n");
