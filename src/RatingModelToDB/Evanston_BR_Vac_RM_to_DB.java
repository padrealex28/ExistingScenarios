...
public class Evanston_BR_Vac_RM_to_DB {
	...
	public static void main(String[] args) throws Exception {
		...
		FileInputStream fis = new FileInputStream("D:\\Rate Matchers\\Markel(Evanston) Rating Model - BR&VAC _ 7.5.22 Ticket.xlsx");
		XSSFWorkbook workbook = new XSSFWorkbook(fis);
		List<String> sheetNames = new ArrayList<String>();
		List<String> rateFactors = new ArrayList<String>();
		logger.info(Thread.currentThread().getStackTrace()[1].getLineNumber() + ": STARTED FOR Evanston BR/Vac RATER ");
		...
		for (int i = 0; i < sheetNames.size(); i++) {
			...
			String query = getQuery.Evanston_BR_Vac_RM_to_DB(i, rateFactors);
			logger.info(
		...
		cq.ComparePremium_Evanston_BR_Vac();	
		System.out.println();
